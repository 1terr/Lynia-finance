...
export async function syncPaymentToFineract(paymentId: string): Promise<void> {
  try {
    ...
    // If this is a deposit (down payment), auto-approve the loan in Fineract
    // Disbursement happens later when distributor completes device handover
    if (payment.payment_type === 'deposit') {
      approveLoanInFineract({
        loanId: payment.loan_id,
        fineractLoanId: loan.fineract_loan_id,
      }).catch((err) => {
        logger.error(`[fineract-sync] Auto-approval failed for loan ${payment.loan_id}`, { action: 'fineract.approve', meta: { error: err instanceof Error ? err.message : String(err) } });
      });
    }
  } catch (error) {
