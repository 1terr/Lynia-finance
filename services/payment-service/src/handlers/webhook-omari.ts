  ...
  try {
    ...
    if (payload.status === 'SUCCESS') {
      ...
      if (resolved) {
        ...
        logger.warn("O'mari webhook: could not resolve payment", {
          action: 'omari.webhook',
    ...
    logger.error("Error processing O'mari webhook", { action: 'omari.webhook', status: 'failed', meta: { error: error instanceof Error ? error.message : 'Unknown' } });
