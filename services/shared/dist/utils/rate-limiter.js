...
exports.getRateLimitCategory = getRateLimitCategory;
...
function getRateLimitCategory(path) {
    ...
    if (path.includes('/otp') || path.includes('/verify'))
        return 'otp';
    if (path.includes('/payment') && !path.includes('/webhook'))
        return 'payment';
 ...
 */
function checkRateLimit(event, categoryOverride) {
    cleanupExpiredEntries();
    const category = categoryOverride || getRateLimitCategory(event.path || '');
    const config = exports.RATE_LIMITS[category] || exports.RATE_LIMITS.api;
 ...
 */
function getRateLimitHeaders(event, categoryOverride) {
    const category = categoryOverride || getRateLimitCategory(event.path || '');
    const config = exports.RATE_LIMITS[category] || exports.RATE_LIMITS.api;
    const clientId = getClientIdentifier(event);
    const key = `${category}:${clientId}`;
