 ...
 * If the build ran without them, the runtime fallback reads from
 * `window.__LYNIA_CONFIG__` which is populated by `public/config.js`
 * (overwritten by CI/CD during deployment).
 */
...
export function createClient() {
  ...
  if (!url || !key) {
    ...
    // actually *used* at build time, so we can safely return null to avoid
    // crashing the build.
    //
    // In the browser, the ConfigGuard component blocks child rendering when
    // config is missing, so downstream code never sees null.
